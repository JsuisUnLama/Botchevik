...
class AudioMeme:
    ...
    @commands.command(name='addmeme', 
                      ...
                      usage="<meme_name> <link>")
    async def addmeme(self, ctx, name, link):
        ...
        try:
            ...
            try:
                ...
                log.error(logMsg)
                await ctx.send(em.standardized_error(errMsg))
                return
    ...
    @commands.command(name='delmeme', 
                      ...
                      usage="<meme_name>")
    async def delmeme(self, ctx, name):
        ...
        try:
            ...
            try:
                ...
                logMsg = "Database error: " + str(e)
                errMsg = "Failed to delete the meme from the database"
                log.error(logMsg)
                await ctx.send(em.standardized_error(errMsg))
                return
    ...
    async def meme(self, ctx, name="h"):
        ...
        if name in ['help','h','getargs','ga']:
            try:
                ...
                embed.add_field(name="Special", value="`random / r`", inline=True)
            ...
            try:
                ...
                if name in ['random','r']:
                    ...
                    max_rand_int = cursor.fetchone()
                    rand = randint(0,max_rand_int[0]-1)
                    cursor.execute("SELECT link_meme FROM memes WHERE id_meme = ?", (rand,))
                    response = cursor.fetchone()
                else:
                    cursor.execute("SELECT link_meme FROM memes WHERE name_meme = ?", (name,))
                    response = cursor.fetchone()
                ...
                # Trigger meme
                print("prout")
                cursor.execute("SELECT is_special FROM memes WHERE name_meme = ?", (name,))
                is_special = cursor.fetchone()
                if is_special[0] is 1:
                    print("ahah yes!")
                    link = ys.from_url(self, link, loop=None, stream=None)

                discord.FFmpegPCMAudio(link)
                await vc.disconnect()
                return
